...
class GameBoard(object):
	...
	def test_play_move(self,move,player,players_color):
		'''
		:param move: position where the move is made [x,y]
		:param player: player that made the move
		'''
		...
		for i in range(len(dx)):
			if self.confirm_direction(move,dx[i],dy[i],player,players_color):
				turn.extend(self.get_changed_stones_in_direction(move,dx[i],dy[i],player,players_color))
		return turn
	...
	def change_stones_in_direction(self,move,dx,dy,player,players_color):
		...
		while (not(self.board[posx][posy] == players_color)):
			...
			posx += dx
			posy += dy
	def get_changed_stones_in_direction(self,move,dx,dy,player,players_color):
		posx = move[0]+dx
		posy = move[1]+dy
		while (not(self.board[posx][posy] == players_color)):
			yield (posx,posy)
